...
static int dbmdx_external_amodel_put(struct snd_kcontrol *kcontrol, // JiangdePending
				 const unsigned int __user *bytes,
				 unsigned int size)
{
	struct dbmdx_private *p = dbmdx_data;
...
static int dbmdx_common_probe(struct dbmdx_private *p)
{
	...
	if (!gpio_is_valid(gpio_vdd)) {
		dev_err(p->dev, "%s: VDD core invalid\n", __func__);
		ret = -EINVAL;
		goto err_clk_off;
	}

	ret = gpio_request(gpio_vdd, "VDD core");
	if (ret < 0) {
		dev_err(p->dev, "%s: error requesting VDD core gpio\n", __func__);
		goto err_clk_off;
	}
	gpio_direction_output(gpio_vdd, 0);
	gpio_set_value(gpio_vdd, 1);
    // Jiangde++, vdd pwr on
	...
	*active_interface = DBMDX_INTERFACE_NONE;
	...
	if (i2c_client) {
		...
		*active_interface = DBMDX_INTERFACE_I2C;
    	printk("%s: hjddbg, has i2c \n", __func__);
	}

	printk("%s: hjddbg, check uart \n", __func__);
	...
	if (uart_client) {
		...
		*active_interface = DBMDX_INTERFACE_UART;
    	printk("%s: hjddbg, has uart \n", __func__);
	}

	printk("%s: hjddbg, check spi \n", __func__);
	...
	if (spi_dev) {
		...
		*active_interface = DBMDX_INTERFACE_SPI;
    	printk("%s: hjddbg, has spi \n", __func__);
	}

	*chip = c;

    printk("%s: hjddbg, interface END 1819 \n", __func__);
	return c ? 0 : -EINVAL;
}
...
static int dbmdx_interface_probe_multi(struct dbmdx_private *p)
{
	...
	dev_err(p->dev, "%s: hjddbg, dbmdx_interface_probe_multi 1 \n", __func__);
	...
	do {
    	...
    	dev_err(p->dev, "%s: hjddbg, dbmdx_find_chip_interface 1 \n", __func__);
...
static int dbmdx_interface_probe_multi(struct dbmdx_private *p)
{
	...
	dev_err(p->dev, "%s: hjddbg, dbmdx_interface_probe_multi 2 \n", __func__);
...
static int dbmdx_platform_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		dev_err(p->dev, "%s: hjddbg, no spi i2c uart \n", __func__);
		goto out_err_free_pdata;
	}
