...
//Fred
unsigned int FTS_CHIP_TYPE = _FT8006P;
bool FTS_CHIP_IDC = false;
bool FTS_HID_SUPPORTTED = false;
...
//Fred add 20190521
    if (chip_id[0] == 0x11)
        FTS_CHIP_TYPE = _FT8006P;
    else if (chip_id[0] == 0x0B)
        FTS_CHIP_TYPE = _FT8006S;

    FTS_CHIP_IDC = ((FTS_CHIP_TYPE & FLAGBIT(FLAG_IDC_BIT)) == FLAGBIT(FLAG_IDC_BIT));
//fix me, what's the purpose of FTS_HID_SUPPORTTED?
    FTS_HID_SUPPORTTED = ((FTS_CHIP_TYPE & FLAGBIT(FLAG_HID_BIT)) == FLAGBIT(FLAG_HID_BIT));
