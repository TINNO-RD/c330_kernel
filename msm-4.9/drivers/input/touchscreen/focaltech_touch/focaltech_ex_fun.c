...
int store_tp_info_c330(void) 
{
    ...
    struct input_dev *input_dev = fts_data->input_dev;
    mutex_lock(&input_dev->mutex);
	fts_read_reg(FTS_REG_VENDOR_ID, &vendor_info);
    fts_read_reg(FTS_REG_FW_VER, &tp_ver_info);

	
    mutex_unlock(&input_dev->mutex);
    if(vendor_info == 0x82) {
      sprintf(buf_temp, "%s-%s-%s-v%02x","hlt","c330ae","ft8006",tp_ver_info);
	  return store_tp_info(buf_temp);
    } else {
       return sprintf(buf_temp,"%s","unknown");
    } 
}
