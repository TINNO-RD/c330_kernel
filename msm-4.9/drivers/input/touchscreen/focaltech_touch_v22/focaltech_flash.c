...
#define FTS_CTP_VENDOR_TCL          (0xE0)
#define FTS_CTP_VENDOR_TXD          (0xF0)

u8 v11cnlite_fw_yeji[] = {
#include "./include/firmware/5446_Ref_V11BN_V01_D01_20170605_app.i"
};

u8 v11cnlite_fw_dijing[] = {
#include "./include/firmware/TL_V11_DJ0x67_FT5446_720x1440_V10_D01_20170721_app.i"
};

u8 v12bn_fw_yeji[] = {
#include "./include/firmware/FT5346DQQ_V12BN_ID0X80_V01_D01_20170614_app.i"
};
...
u8 c860_fw_dijing[] = {
#include "./include/firmware/TL_C860_DJ_FT5446_720x1528_V13_20180112_app.i"
};
...
u8 p201as_fw_tcl[] = {
#include "./include/firmware/P201_TINNO_FT8613_5P99_Pid0xE0_Vid0xE0_V14_D01_20180524_app.i"
};
...
u8 p220an_fw_holitech[] = {
#include "./include/firmware/P220_TINNO_FT3427_HLTL0x82_V05_D01_20180412_app.i"
};

u8 p220an_fw_tcl[] = {
#include "./include/firmware/P220_TINNO_FT3427_TCL0xE0_V07_D01_20180420_app.i"
};

u8 x600as_fw_tcl[] = {
#include "./include/firmware/X600AS_FT8719_FW_CSOT_6.18inch_V0x01_D0x01_20180206_app.i"
};

u8 p700as_fw_txd[] = {
#include "./include/firmware/P700_TINNO_FT8719_TXD0xF0_PID0xC3_FTS8719P001_VER0x0A_I2C_20180823_app.i"
};

struct tp_fw_map tinno_tp_fw_map[] =
{
	{"v11cnlite",	FTS_CTP_VENDOR_YEJI,		0,	"YEJI",		v11cnlite_fw_yeji,	sizeof(v11cnlite_fw_yeji)	},
	{"v11cnlite",	FTS_CTP_VENDOR_DIJING,		0,	"DIJING",	v11cnlite_fw_dijing,	sizeof(v11cnlite_fw_dijing)	},
	{"v12bn",	FTS_CTP_VENDOR_YEJI,		0,	"YEJI",		v12bn_fw_yeji,		sizeof(v12bn_fw_yeji)		},
	{"v12bnlite",	FTS_CTP_VENDOR_YEJI,		0,	"YEJI",		v12bnlite_fw_yeji,	sizeof(v12bnlite_fw_yeji)	},
	{"v12bnlite",	FTS_CTP_VENDOR_HOLITECH,	0,	"HOLITECH",	v12bnlite_fw_holitech,	sizeof(v12bnlite_fw_holitech)	},
	{"v12fnlite",	FTS_CTP_VENDOR_YEJI,		0,	"YEJI",		v12fnlite_fw_yeji,	sizeof(v12fnlite_fw_yeji)	},
	{"v12fnlite",	FTS_CTP_VENDOR_HOLITECH,	0,	"HOLITECH",	v12fnlite_fw_holitech,	sizeof(v12fnlite_fw_holitech)	},
	{"c860",	FTS_CTP_VENDOR_DIJING,		0,	"DIJING",	c860_fw_dijing,		sizeof(c860_fw_dijing)		},
	{"c860",	FTS_CTP_VENDOR_YEJI,		0,	"YEJI",		c860_fw_yeji,		sizeof(c860_fw_yeji)		},
	{"p201as",	FTS_CTP_VENDOR_TCL,		0x8613,	"TCL",		p201as_fw_tcl,		sizeof(p201as_fw_tcl)		},
	{"p201as",	FTS_CTP_VENDOR_TCL,		0x8716,	"TCL",		p201as_fw_tcl2,		sizeof(p201as_fw_tcl2)		},
	{"p210cn",	FTS_CTP_VENDOR_HOLITECH,	0,	"HOLITECH",	p220an_fw_holitech,	sizeof(p220an_fw_holitech)	},
	{"p210ta30",	FTS_CTP_VENDOR_HOLITECH,	0,	"HOLITECH",	p220an_fw_holitech,	sizeof(p220an_fw_holitech)	},
	{"p220an",	FTS_CTP_VENDOR_HOLITECH,	0,	"HOLITECH",	p220an_fw_holitech,	sizeof(p220an_fw_holitech)	},
	{"p220an",	FTS_CTP_VENDOR_TCL,		0,	"TCL",		p220an_fw_tcl,		sizeof(p220an_fw_tcl)		},
	{"x600as",	FTS_CTP_VENDOR_TCL,		0,	"TCL",		x600as_fw_tcl,		sizeof(x600as_fw_tcl)		},
	{"p700as",	FTS_CTP_VENDOR_TXD,		0,	"TXD",		p700as_fw_txd,		sizeof(p700as_fw_txd)		},
	{"Snapdragon",	FTS_CTP_VENDOR_YEJI,		0,	"YEJI",		v12bnlite_fw_yeji,	sizeof(v12bnlite_fw_yeji)	}, //default
...
#if FTS_AUTO_LIC_UPGRADE_EN
if (ts_data->pdata->display_cfg_auto_update) {
	...
	for(p = tinno_tp_fw_map; p->project_name != NULL; p++){
		if((!strcmp(p->project_name, project_name)) &&
		   (p->tp_vendor_id == vendor_id)){
			...
			if (fts_data->pdata->multiple_ic)
			{
				unsigned int ic_type = (unsigned int)fts_data->ic_info.ids.chip_idh << 8 | fts_data->ic_info.ids.chip_idl;
				if (p->ic_type == ic_type)
					FTS_INFO("[UPGRADE]tp IC type = 0x%x", ic_type);
				else {
					//FTS_INFO("[UPGRADE]FW IC type = 0x%x, tp IC type = 0x%x, not match",
					//	p->ic_type, ic_type);
					continue;
				}
			}

			fw->fw_file = p->fw_array;
			fw->fw_len  = p->fw_len;
...
char* tinno_get_project_tp_vendor(char vendor_id)
{
	struct tp_fw_map *p;
	for(p = tinno_tp_fw_map; p->project_name != NULL; p++){
...
//	char *project_name = tinno_get_project_name();
	char *project_name = "demo";
	ret = tinno_get_project_tp_fw(project_name, vendor_id&0xFF);
	if (ret){
		FTS_ERROR("no vendor id match, don't get file");
#if 0
		save_tp_info();
...
char* tinno_get_project_tp_vendor(char vendor_id);
extern unsigned int FTS_CHIP_TYPE;
static int save_tp_info(void)
{
        char buf[256];
        ...
        fts_fwupg_get_vendorid(fts_data, &vendor_id);
        fts_fwupg_get_ver_in_tp(fts_data->client, &ver);

        vendor_name = tinno_get_project_tp_vendor(vendor_id&0xFF);
        FTS_INFO("vendor id = %x, vendor name = %s", vendor_id, vendor_name);
if (fts_data->pdata->multiple_ic) {
        if (vendor_name){
            sprintf(buf, "%s-%s-FT%02X%02X-V%02X", vendor_name,
                tinno_get_project_name(),
                fts_data->ic_info.ids.chip_idh, fts_data->ic_info.ids.chip_idl,
                ver);
        } else {
            sprintf(buf, "%s-%s-FT%02X%02X-V%02X", "UNKNOWN",
                tinno_get_project_name(),
                fts_data->ic_info.ids.chip_idh, fts_data->ic_info.ids.chip_idl,
                ver);
        }
        ...
        if (vendor_name){
            sprintf(buf, "%s-%s-FT%04X-V%02X", vendor_name,
                tinno_get_project_name(),
                (FTS_CHIP_TYPE >> 16),
                ver);
        } else {
            sprintf(buf, "%s-%s-FT%04X-V%02X", "UNKNOWN",
                tinno_get_project_name(),
                (FTS_CHIP_TYPE >> 16),
                ver);
        }
