...
#define IC_TO_FTS_CHIP_IDC(x)            (((x) & FLAGBIT(FLAG_IDC_BIT)) == FLAGBIT(FLAG_IDC_BIT))
#define IC_TO_FTS_HID_SUPPORTTED(x)      (((x) & FLAGBIT(FLAG_HID_BIT)) == FLAGBIT(FLAG_HID_BIT))
...
#define FTS_DEBUG(fmt, args...) if(global_debug_flag) printk("[FTS][%s]"fmt"\n", __func__, ##args); else 
#else
#define FTS_DEBUG(fmt, args...) if(global_debug_flag) printk("[FTS]"fmt"\n", ##args); else 
#endif
#define FTS_FUNC_ENTER() if(global_debug_flag) printk("[FTS]%s: Enter\n", __func__); else 
#define FTS_FUNC_EXIT()  if(global_debug_flag) printk("[FTS]%s: Exit(%d)\n", __func__, __LINE__); else 
