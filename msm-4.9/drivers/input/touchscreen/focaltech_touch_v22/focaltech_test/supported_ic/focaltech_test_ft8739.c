...
*
* File Name: Focaltech_test_ft8739.c
*
...
enum test_item_ft8739 {
    CODE_FT8739_ENTER_FACTORY_MODE = 0,
    CODE_FT8739_RAWDATA_TEST = 7,
    CODE_FT8739_CB_TEST = 12,
    CODE_FT8739_SHORT_CIRCUIT_TEST = 14,
    CODE_FT8739_LCD_NOISE_TEST = 15,
    CODE_FT8739_OPEN_TEST = 24,
};
...
*****************************************************************************/
struct ft8739_basic_threshold ft8739_basic_thr;
struct ft8739_test_item ft8739_item;

/*****************************************************************************
* Static function prototypes
*****************************************************************************/
static int ft8739_get_key_num(void)
{
    int ret = 0;
    int i = 0;
    u8 keyval = 0;
...
static int ft8739_enter_factory_mode(void)
{
    int ret = 0;
...
static int ft8739_short_test(bool *test_result)
{
    int ret = 0;
    ...
    for (row = 0; row < tx_num + 1; ++row) {
        for (col = 0; col < rx_num; ++col) {
            tmp_adc = adcdata[row * rx_num + col] / 4;
            if (tmp_adc <= 1900) {
                adcdata[row * rx_num + col] = -1;
                continue;
            } else if (tmp_adc < 2108) {
                tmp_adc = 2108;
            }
            adcdata[row * rx_num + col] = 184320 / (tmp_adc - 2048) - 60;
        }
    }
    ...
    /* analyze data */
    res_min = ft8739_basic_thr.short_res_min;
    min = res_min;
    max = 100000000;
...
static int ft8739_open_test(bool *test_result)
{
    int ret = 0;
    ...
    /* bit 1~0 = 2'b10 */
    tmp_val = (reg_vgh & 0xFE) | 0x02;
    FTS_TEST_DBG("reg20=0x%x\n", tmp_val);
    ret = write_reg(FACTORY_REG_VGH02, tmp_val);
    if (ret) {
        FTS_TEST_SAVE_ERR("\r\nwrite reg 0x20 fail");
        goto TEST_ERR_FACTORY;
    }

    if (ft8739_basic_thr.open_test_k1_check) {
        ret = read_reg(FACTORY_REG_K1, &k1);
        ret = write_reg(FACTORY_REG_K1, ft8739_basic_thr.open_test_k1);
        if (ret) {
            FTS_TEST_SAVE_ERR("\r\nwrite reg k1 fail");
            goto TEST_ERR_FACTORY;
        }
    ...
    if (ft8739_basic_thr.open_test_k2_check) {
        ret = read_reg(FACTORY_REG_K2, &k2);
        ret = write_reg(FACTORY_REG_K2, ft8739_basic_thr.open_test_k2);
        if (ret) {
            FTS_TEST_SAVE_ERR("\r\nwrite reg k2 fail");
            goto TEST_ERR_FACTORY;
        }
    ...
    /* compare data */
    min = ft8739_basic_thr.open_test_cb_min;
...
static int ft8739_cb_test(bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int tx_num = 0;
    ...
    include_key = ft8739_basic_thr.cb_test_vk_check;
...
static int ft8739_rawdata_test(bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int tx_num = 0;
...
static int ft8739_lcdnoise_test(bool *test_result)
{
    int ret = 0;
    ...
    /* set scan number */
    frame_num = ft8739_basic_thr.lcd_noise_test_frame;
    ...
    value_min = 0;
    value_max = ft8739_basic_thr.lcd_noise_coefficient * noise_value_va * 32 / 100;
    vk_value_max = ft8739_basic_thr.lcd_noise_coefficient_key * noise_value_vk * 32 / 100;
...
static bool start_test_ft8739(void)
{
    int ret = 0;
    ...
    for (item_count = 0; item_count < test_data.test_num; item_count++) {
        ...
        /* FT8739_ENTER_FACTORY_MODE */
        if (CODE_FT8739_ENTER_FACTORY_MODE == item_code) {
            ret = ft8739_enter_factory_mode();
        ...
        /* FT8739_SHORT_TEST */
        if (CODE_FT8739_SHORT_CIRCUIT_TEST == item_code) {
            ret = ft8739_short_test(&temp_result);
            if ((ret != 0) || (!temp_result)) {
                test_result = false;
                test_data.test_item[item_count].testresult = RESULT_NG;
            } else
        ...
        /* FT8739_Open_TEST */
        if (CODE_FT8739_OPEN_TEST == item_code) {
            ret = ft8739_open_test(&temp_result);
            if ((ret != 0) || (!temp_result)) {
                test_result = false;
                test_data.test_item[item_count].testresult = RESULT_NG;
            } else
        ...
        /* FT8739_CB_TEST */
        if (CODE_FT8739_CB_TEST == item_code) {
            ret = ft8739_cb_test(&temp_result);
            if ((ret != 0) || (!temp_result)) {
                test_result = false;
                test_data.test_item[item_count].testresult = RESULT_NG;
            } else
        ...
        /* FT8739_RAWDATA_TEST */
        if (CODE_FT8739_RAWDATA_TEST == item_code) {
            ret = ft8739_rawdata_test(&temp_result);
            if ((ret != 0) || (!temp_result)) {
                test_result = false;
                test_data.test_item[item_count].testresult = RESULT_NG;
            } else
        ...
        /* FT8739_LCD_NOISE_TEST */
        if (CODE_FT8739_LCD_NOISE_TEST == item_code) {
            ret =  ft8739_lcdnoise_test(&temp_result);
            if ((ret != 0) || (!temp_result)) {
                test_result = false;
                test_data.test_item[item_count].testresult = RESULT_NG;
            } else
...
static void init_basicthreshold_ft8739(char *ini)
{
...
static void init_testitem_ft8739(char *ini)
{
    ...
    ft8739_item.rawdata_test = fts_atoi(str);

    /* CB_TEST */
    GetPrivateProfileString("TestItem", "CB_TEST", "1", str, ini);
    ft8739_item.cb_test = fts_atoi(str);

    /* SHORT_CIRCUIT_TEST */
    GetPrivateProfileString("TestItem", "SHORT_CIRCUIT_TEST", "0", str, ini);
    ft8739_item.short_test = fts_atoi(str);

    /* LCD_NOISE_TEST */
    GetPrivateProfileString("TestItem", "LCD_NOISE_TEST", "0", str, ini);
    ft8739_item.lcd_noise_test = fts_atoi(str);

    /* OPEN_TEST */
    GetPrivateProfileString("TestItem", "OPEN_TEST", "0", str, ini);
    ft8739_item.open_test = fts_atoi(str);

    FTS_TEST_FUNC_EXIT();
}

static void init_detailthreshold_ft8739(char *ini)
{
    FTS_TEST_FUNC_ENTER();
...
static void set_testitem_sequence_ft8739(void)
{
    FTS_TEST_FUNC_ENTER();

    test_data.test_num = 0;
    /* Enter Factory Mode */
    fts_set_testitem(CODE_FT8739_ENTER_FACTORY_MODE);

    /* Short Test */
    if ( ft8739_item.short_test == 1) {
        fts_set_testitem(CODE_FT8739_SHORT_CIRCUIT_TEST);
    }

    /* Open Test */
    if ( ft8739_item.open_test == 1) {
        fts_set_testitem(CODE_FT8739_OPEN_TEST);
    }

    /* CB_TEST */
    if ( ft8739_item.cb_test == 1) {
        fts_set_testitem(CODE_FT8739_CB_TEST);
    }

    /* RawData Test */
    if ( ft8739_item.rawdata_test == 1) {
        fts_set_testitem(CODE_FT8739_RAWDATA_TEST);
    }

    /* LCD_NOISE_TEST */
    if ( ft8739_item.lcd_noise_test == 1) {
        fts_set_testitem(CODE_FT8739_LCD_NOISE_TEST);
    }

    FTS_TEST_FUNC_EXIT();
}

struct test_funcs test_func_ft8739 = {
    .ic_series = TEST_ICSERIES(IC_FT8739),
    .init_testitem = init_testitem_ft8739,
    .init_basicthreshold = init_basicthreshold_ft8739,
    .init_detailthreshold = init_detailthreshold_ft8739,
    .set_testitem_sequence  = set_testitem_sequence_ft8739,
    .start_test = start_test_ft8739,
};
