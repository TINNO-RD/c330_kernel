...
unsigned char FT6X36_TestItem_RawDataTest(bool *bTestResult)
{
    ...
    RawDataMin = g_stCfg_FT6X36_BasicThreshold.rawdata_test_min;
    RawDataMax = g_stCfg_FT6X36_BasicThreshold.rawdata_test_max;
    ...
    for (i = 0; i < test_data.screen_param.channels_num + test_data.screen_param.key_num; i++) {
        RawDataMin = test_data.scap_detail_thr.rawdata_test_min[i];//Fetch detail threshold
        RawDataMax = test_data.scap_detail_thr.rawdata_test_max[i];//Fetch detail threshold
...
unsigned char FT6X36_TestItem_CbTest(bool *bTestResult)
{
    ...
    iCbMin = g_stCfg_FT6X36_BasicThreshold.cb_test_min;
    iCbMax = g_stCfg_FT6X36_BasicThreshold.cb_test_max;
    ...
    if ((ERROR_CODE_OK != ReCode)/* || (1 != WaterProofResult)*/) {
        ...
        for (i = 0; i < test_data.screen_param.channels_num + test_data.screen_param.key_num; i++) {
            ...
            iCbMin = test_data.scap_detail_thr.cb_test_min[i];//Fetch detail threshold
            iCbMax = test_data.scap_detail_thr.cb_test_max[i];//Fetch detail threshold
...
void init_basicthreshold_ft6x36(char *strIniFile)
{
    ...
    g_stCfg_FT6X36_BasicThreshold.rawdata_test_min = fts_atoi(str);
    GetPrivateProfileString("Basic_Threshold", "RawDataTest_Max", "17000", str, strIniFile);
    g_stCfg_FT6X36_BasicThreshold.rawdata_test_max = fts_atoi(str);
    ...
    g_stCfg_FT6X36_BasicThreshold.cb_test_min = fts_atoi(str);
    GetPrivateProfileString("Basic_Threshold", "CbTest_Max", "250", str, strIniFile);
    g_stCfg_FT6X36_BasicThreshold.cb_test_max = fts_atoi(str);
...
void init_detailthreshold_ft6x36(char *ini)
{
    ...
    memcpy(test_data.scap_detail_thr.rawdata_test_max, test_data.scap_detail_thr.TempData, MAX_CHANNEL_NUM * sizeof(int));

    OnGetTestItemParam("RawDataTest_Min", ini, 16500);
    memcpy(test_data.scap_detail_thr.rawdata_test_min, test_data.scap_detail_thr.TempData, MAX_CHANNEL_NUM * sizeof(int));
    ...
    memcpy(test_data.scap_detail_thr.cb_test_max, test_data.scap_detail_thr.TempData, MAX_CHANNEL_NUM * sizeof(int));

    OnGetTestItemParam("CbTest_Min", ini, 0);
    memcpy(test_data.scap_detail_thr.cb_test_min, test_data.scap_detail_thr.TempData, MAX_CHANNEL_NUM * sizeof(int));
...
struct test_funcs test_func_ft6x36 = {
    .ic_series = TEST_ICSERIES(IC_FT6X36),
